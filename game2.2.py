...
def load_enemy_images():
    enemy_images = []
    for i in range(1, 4):  # Загружаем 3 изображения для врагов
        image_path = os.path.join("Sprites", f"enemy_tank_{i}.png")
        if not os.path.exists(image_path):
            raise FileNotFoundError(f"Файл {image_path} не найден!")
        image = pygame.image.load(image_path)
        image = pygame.transform.scale(image, (TANK_WIDTH, TANK_HEIGHT))
...
def main():
    ...
    enemies = [
        ...
        for i in range(3)  # Создаем 3 врагов с разными изображениями
    ]
